...
from src.app.services.nosqlConnection import NoSQLConnectionManager as cm, NoSQLTableManager as tm
...
class PromptConfigManager:
    ...
    @classmethod
    async def get_prompts_data(cls, nosql_conn, logger):
        try:
            prompt_config_data = tm.execute_select_query(
                nosql_conn,
                "select * from WAIAgentPromptsConfig"
            )

            return prompt_config_data
        except Exception as e:
            logger.error(f"Failed to get the prompts configuration data from NoSQL DB. Error: {e}")
            return []
    ...
    async def manage_prompts_config(cls, prompts_config_data: list, handler, logger):
        ...
        if error:
            ...
            return "SUCCESS: Data updated to DB."
...
class CustomerProcessDetailsConfigManager:
    """
    A class to manage prompt configurations for NoSQL databases.
    ...
    @classmethod
    async def get_process_details_data(cls, nosql_conn, logger):
        try:
            process_details_config_data = tm.execute_select_query(
                nosql_conn,
                "select * from CustomerProcessDetails"
            )

            return process_details_config_data
        except Exception as e:
            logger.error(f"Failed to get the prompts configuration data from NoSQL DB. Error: {e}")
            return []

    @classmethod
    async def manage_process_details_config(cls, process_details_config_data: list, handler, logger):
        """
        Handles the insertion, update, or deletion of process details configurations.

        Args:
            process_details_config_data (list): A list of process details configuration dictionaries.
            handler: The database connection handler.
            logger: Logger instance for logging errors and information.
        ...
        """
        error = None
        for each_process_details_config in process_details_config_data:
            try:
                operation_flag = each_process_details_config.get('operation_flag')
                cls._wai_process_details_config_manager(each_process_details_config, operation_flag, handler, logger)
            except Exception as e:
                logger.error(f"Failed in insert/update/delete prompts config details. Error: {e}")
                error = e
    ...
    @classmethod
    def _wai_process_details_config_manager(cls, process_details_config: dict, operation_flag: str, handler, logger):
        """
        Manages individual process details configurations based on the operation flag.

        Args:
            process_details_config (dict): The process details configuration data.
            operation_flag (str): The operation to perform (insert, update, delete).
            handler: The database connection handler.
            logger: Logger instance for logging errors and information.
        ...
        if operation_flag.lower() == "insert" or operation_flag.lower() == "i":
            process_details_config["creation_time"] = now_utc
            process_details_config["last_updated"] = now_utc
            try:
                process_details_config['customer_process_detail_id'] = tm.get_next_sequence_id(handler, 'CustomerProcessDetailsIdSeq')
                tm.execute_insert_query(handler, process_details_config, 'CustomerProcessDetails')
                logger.info("Insertion of prompt details is successful.")
            except Exception as e:
                logger.error(f"Failed to insert config data - \n{process_details_config}\n\nError: {e}")
                logger.error("Insertion of customer process details is failed.")
                raise Exception(f"Insertion failed. Error: {e}")

        elif operation_flag.lower() == "update" or operation_flag.lower() == "u":
            process_details_config["last_updated"] = now_utc
            try:
                tm.execute_update_query(handler, process_details_config, 'CustomerProcessDetails')
                logger.info("Update of prompt details is successful.")
            except Exception as e:
                logger.error(f"Failed to update config data - \n{process_details_config}\n\nError: {e}")
                logger.error("Update of customer process details is failed.")
                raise Exception(f"Update failed. Error: {e}")

        elif operation_flag.lower() == 'delete' or operation_flag.lower() == 'd':
            try:
                tm.execute_delete_query(handler, process_details_config, 'CustomerProcessDetails')
                logger.info("Deletion of prompt details is successful.")
            except Exception as e:
                logger.error(f"Failed to delete config data - \n{process_details_config}\n\nError: {e}")
                logger.error("Deletion of customer process details is failed.")
                raise Exception(f"Deletion failed. Error: {e}")
        else:
            raise ValueError("operation flag must be 'insert' or 'i' or 'update' or 'u' or 'delete' or 'd'")
...
if __name__ == '__main__':
    ...
    prompt_config_l = [
  {
    ...
    asyncio.run(PromptConfigManager.manage_prompts_config(prompt_config_l, l_handler, l_logger))
