...
from src.app.services.vertixAIActivities import MimeTypes as mt
...
class ProcesTicketsDB:
    @classmethod
    def _get_existing_tickets(cls, db_cursor, logger, product_name = None, customer_name = None):
        ...
        tickets_query += f" and product_name = '{product_name}'" if product_name else ''
        tickets_query += f" and customer_name = '{customer_name}'" if customer_name else ''
        ...
        try:
            ...
            logger.error(
                f"Failed to get the existing tickets from DB. Error details: {e}.\ntickets_query: {tickets_query}"
            )

        return tickets

    @classmethod
    def _update_ticket_details(cls, ticket_details, customer_name, db_cursor, logger):
    ...
    @classmethod
    def _insert_ticket(cls, ticket_details, existing_tickets, application_db_conn, logger):
        insert_ticket = """
        ...
        print(f"ticket_details: {ticket_details}")
        with application_db_conn.cursor() as db_cursor:
            if ticket_details.get('jira_ticket_id') not in existing_tickets:
                ...
                cls._update_ticket_details(ticket_details, ticket_details.get('customer_name'), db_cursor, logger)

    @classmethod
    def _store_tickets_db(cls, tickets, application_db_conn, logger):
        logger.info("Storing tickets into db..")
        with application_db_conn.cursor() as db_cursor:
            existing_tickets = cls._get_existing_tickets(db_cursor, logger)
...
"""
    ...
    @classmethod
    def _get_process_flow(cls, product_name, customer_name, process_name, nosql_conn, logger):
...
class JiraActivities (AttachmentProcessor, ProcesTicketsDB):
    ...
    def fetch_filtered_issues(
            cls, jira, project_name, status, assignee, p_closed_ticket_ids, exclude_tickets, logger, chunk_size=300
    ):
        base_jql = f'project = "{project_name}"'
        if status.lower() != "all":
            base_jql += f' AND status = "{status}"' if status else ' AND status IN ("NEW", "InProgress")'
        if assignee:
            base_jql += f' AND assignee = "{assignee}"'

        results = []
        if exclude_tickets and p_closed_ticket_ids:
            for chunk in chunked(p_closed_ticket_ids, chunk_size):
                ticket_str = ", ".join(f'"{tid}"' for tid in chunk)
                chunk_jql = base_jql + f" AND key NOT IN ({ticket_str})"
                logger.info(f"Executing chunked JQL: {chunk_jql}")
                issues = jira.search_issues(chunk_jql, maxResults=1000)
                results.extend(issues)
        else:
            full_jql = base_jql
            if p_closed_ticket_ids:
                ticket_str = ", ".join(f'"{tid}"' for tid in p_closed_ticket_ids)
                condition = "NOT IN" if exclude_tickets else "IN"
                full_jql += f" AND key {condition} ({ticket_str})"
            logger.info(f"Executing single JQL: {full_jql}")
            results = jira.search_issues(full_jql, maxResults=1000)
    ...
    @classmethod
    def get_tickets(cls, product_name, server_url, jira_user_name, jira_api_token, application_db_conn, logger,
                    project_name, project_key, status='',  assignee='', p_closed_ticket_ids=None,
                    p_not_closed_ticket_ids=None, exclude_tickets=False,
        ...
        if p_not_closed_ticket_ids is None:
            p_not_closed_ticket_ids = list()
        try:
            logger.info("Fetching new tickets")
            with application_db_conn.cursor() as db_cursor:
                existing_tickets = cls._get_existing_tickets(
                    db_cursor,
                    logger,
                ...
                issues = cls.fetch_filtered_issues(
                    jira,
                    project_key,
                    status,
                    assignee,
    ...
    def _process_tickets(
        ...
        tickets_query = (
            ...
            "from support_tickets "
            # "where TICKET_ID > 3168 and DESCRIPTION is not null"
        ...
        with application_db_conn.cursor() as application_db_cursor:
            ...
            for each_ticket in all_tickets:
                ...
                try:
                    ...
                    process_flow = cls._get_process_flow(product_name, customer_name, ag1_resp.get('process_name'), nosql_conn, logger)
                    ...
                    ai_resolution = sa.Agents.complete_response_analyzer(
                        ag2_resp, jira_ticket_id, ticket_description, '', ag1_resp.get('process_name'), product_name,
                    ...
                    if ai_resolution:
                        ...
                        print(f"ai_res_comments: {ai_resolution}")
                        cls._update_ticket_details(ai_res_comments, customer_name, application_db_cursor, logger)
                    # break
                    logger.info(f"Issue - {ticket_id}")
                except Exception as e:
                    logger.error(f"Failed to process the issue - {ticket_id}. \nError: {e}")
    ...
    async def jira_support_agents(
        ...
        for each_project in projects:
            try:
                ...
                project_key = each_project['jira_project_key']
                ...
                cls.get_tickets(
                    product_name, server, jira_user_name, jira_api_token, application_db_conn, logger,
                    project_name=project_name, project_key=project_key, status=status, assignee=assignee,
                    p_closed_ticket_ids=r_closed_ticket_ids,
