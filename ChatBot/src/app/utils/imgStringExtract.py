...
from src.app.utils.dataValidation import DataSanitizer as ds
...
class GoogleVisionOCR:
    ...
    @classmethod
    def sort_json(cls, raw_json_data, sort_type):
        """
    ...
    def json_str(cls, sorted_json_data, logger, res_str=''):
        ...
        row_str = '"'
        row_data = cls.sort_json(sorted_json_data, 'x')
    ...
    @classmethod
    def get_string(cls, img_file, b64_flg, logger, data_end_str=None, i=1):
        """
        ...
        try:
            ...
            if response and response.status_code == 200 and not response.json().get('error'):
                ...
                sorted_json = cls.sort_json(img_json_response['textAnnotations'][1:], 'y')
    ...
    @classmethod
    def get_img_string(cls, img_file_path, logger, data_end_str=None):
        """
        Reads an image from file and extracts text.
        """
        with open(img_file_path, 'rb') as f:
            img = f.read()
            _, result = cls.get_string(img, False, logger, data_end_str=data_end_str)
        return result
...
if __name__ == "__main__":
    ...
    # Example usage
    extracted_text = GoogleVisionOCR.get_img_string(image_path, l_logger)
