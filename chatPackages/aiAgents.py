...
from chatPackages.prompts import VertexAIService as vai
from chatPackages.dataValidation import Utils as ut
from chatPackages.embeddingActivites import EmbeddingManager as em
...
#                 system_instruction = """
# **Objective:**
# 	Synthesize a concise, contextually relevant, and continuous answer to the user's `main_question`, leveraging the provided `context` and `previous_conversation` (if available), and format the response using Markdown. Limit the process to a maximum of two iterations to ensure completeness.
#
# **Task:**
# 	Generate a coherent and informative response that addresses the user's query, building upon previous interactions and utilizing the provided context.
#
# **Instructions:**
# 1.  **Input Reception:**
#     * Receive the following inputs:
#         * `main_question` (string): The user's original query.
#         * `context` (string): Relevant information for answering the question.
#         * `previous_conversation` (array, optional): A chronological array of up to three previous conversation turns, each containing `user` and `agent` fields. This will be null or empty for new conversations.
#
# 2.  **Contextual Analysis:**
#     * If `previous_conversation` is provided, analyze it to understand the ongoing dialogue, user intent, and information already exchanged.
#     * Integrate information from `previous_conversation` to refine the understanding of `main_question` and `context`.
#
# 3.  **Iterative Response Generation (Maximum Two Iterations):**
#     * **First Iteration:** Generate an initial response to `main_question` based on the provided `context`.
#     * **Second Iteration (Conditional):**
#         * Proceed to a second iteration only if:
#             * The user explicitly requests a more detailed answer.
#             * The `main_question` necessitates extensive information.
#             * The initial response is demonstrably incomplete or requires elaboration.
#         * The second iteration must build upon the first response, providing additional relevant information and maintaining conversational continuity.
#         * After the second iteration, the generated response **must** be complete and self-contained.
#
# 4.  **Markdown Formatting:**
#     * Format the generated response using Markdown syntax for clarity and readability.
#     * Utilize headings, lists, bold/italic text, code blocks, and other Markdown elements as appropriate.
#
# 5.  **Contextual Accuracy and Relevance:**
#     * Ensure the generated response accurately reflects the information in `context` and is directly relevant to `main_question`, considering the context established by `previous_conversation`.
#     * Apply semantic analysis to identify and utilize closely related content when direct answers are not immediately apparent.
#
# 6.  **Response Continuity:**
#     * Maintain a logical flow and continuity between the first and second iterations.
#     * Avoid repetition or disjointed information in the second iteration.
#
# 7.  **Output:**
#     * Return a string containing the generated response, formatted in Markdown.
#     * Do not include source citations or extraneous information.
#                                 """
        ...
        @classmethod
        def get_winfo_doc_questions_contents(
                cls,
                doc_questions,
                product_name,
                process_name,
            ...
            """"""
            logger.info(
                f"Support chart bot stated with get_winfo_doc_questions_contents. user question: {doc_questions}")
            ...
            try:
                ...
                logger.error(f"Sales chart bot failed with get doc questions contents. error: {e}")
        ...
        @classmethod
        def get_winfo_db_questions_contents(
                cls,
                winfo_db_questions,
                db_cursor,
                logger,
    ...
    {{
      ...
      "customer_process_descriptions": """{customer_process_descriptions}"""
            ...
            ag1_resp = pr.get_prompt_response(
        ...
        @classmethod
        def agent2(cls, previous_chats, ticket_description, customer_id, process_name, process_flow,
    ...
    {{
      ...
      "previous_ticket_interactions": {previous_chats}
            ...
            ag2_resp = pr.get_prompt_response(
        ...
        def agent3(cls, categorized_questions, product_name, process_name, customer_name, ai_db_conn,
            ...
            try:
                winfo_db_questions = categorized_questions.get('winfobots_database')
                oracle_db_questions = categorized_questions.get('customer_oracle_database')
                doc_questions = categorized_questions.get('process_documents')

                if doc_questions:
                    with ai_db_conn.cursor() as ai_db_cursor:
                        doc_resp = cls.get_winfo_doc_questions_contents(
        ...
        @classmethod
        def agent4(cls, ticket_description, resolved_questions, customer_name, product_name, nosql_conn, logger,
                   google_key_config_path='../configuration/Google_Key(WinfoBots).json'):
...
"resolved_questions": {resolved_questions}
            ...
            ag4_resp = pr.get_prompt_response(
...
"chat_history": {previous_chats},
            ...
            ag5_resp = pr.get_prompt_response(
...
{{
    ...
    "support_agent_query": "{support_agent_query}"
            ...
            ag7_resp = pr.get_prompt_response(
