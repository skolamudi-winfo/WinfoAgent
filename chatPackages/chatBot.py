...
from chatPackages.aiAgents import SalesAgent as sa
from chatPackages.aiAgents import SupportAgent as supa
from chatPackages.nosqlConnection import NoSQLTableManager as tm
from chatPackages.pdfStringExtract import PDFProcessor as pdfp
...
class ChatOperations:
    ...
    @classmethod
    def _generate_chat_id(cls, id_length=16):
        import random
        import string
...
class SupportChatBot(ChatOperations):
    ...
    async def support_agent(
        ...
        if not prev_chat:
            ...
            pass
            print("Before Ticket summary call")
            asyncio.create_task(
            ...
            print("Post Ticket summary call")
...
class OracleSupportProcessFiles:
    ...
    class EmbeddingService:
                ...
                with conn.cursor() as db_cursor:
                    for content in all_contents:
                        ...
                        if not insert_status:
                            ...
                            update_content_id = {
                                "content_id": content_id,
                                "content_details": content.get('content_details'),
                                "process_name": process_name,
                                "product_name": product_name,
                                "process_area": process_area,
                                "sub_process": sub_process,
                            ...
                            update_flg = tm.execute_update_query(nosql_conn, update_content_id, 'SupportDocumentsContent')
        ...
        def _insert_questions_db(
            ...
            query_insertion = (
                "insert into support_content_embedding (QUERY_ID, CONTENT_ID, product_name, process_name, process_area, sub_process, embedding) "
                "values (support_content_embedding_id_seq.nextval, :content_id, :product_name, :process_name, :process_area, :sub_process, :embedding)"
            )
            try:
                db_cursor.execute(
                    query_insertion,
...
class WinfoBotsSupportProcessFiles:
    ...
    values(
      ...
      :product_name
        ...
        def _store_content_to_nosql_db(
                cls, content, product_name, process_area, process_name, sub_process, file_name, nosql_conn, logger
        ):
    ...
    class ContentPreparationService(DatabaseService):
        ...
        def _content_preparation_gcp(
                cls, bucket_name, gcs_folder_path, product_name, process_area, process_name, sub_process, nosql_conn, logger,
                google_key_config_path='../configuration/Google_Key(WinfoBots).json'
        ):
            """
            Reads files from GCS, processes their content, and stores the processed data in the database.
        ...
        Important Guidelines:
            ...
            ...
                    ...
                    try:
                        ...
                        ins_status = cls._store_content_to_nosql_db(
                            file_content, product_name, process_area, process_name, sub_process, file_name, nosql_conn, logger
                        )
    ...
    Important Guidelines:
        ...
        @classmethod
        def content_preparation(
                cls, pdf_path, file_name, product_name, process_area, process_name, sub_process, nosql_conn, logger,
                google_key_config_path='../configuration/Google_Key(WinfoBots).json',
                model_name='gemini-2.0-flash-001', location='us-central1'
        ):
            """
            ...
            try:
                ...
                ins_status = cls._store_content_to_nosql_db(
                    pdf_content, product_name, process_area, process_name, sub_process, file_name, nosql_conn, logger
                )
